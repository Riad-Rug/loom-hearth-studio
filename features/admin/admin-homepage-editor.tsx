  ...
  return (
    <div className={styles.stack}>
      <section className={styles.card}>
        ...
        <ToggleField checked={section.visible} label="Show section on homepage" onChange={(value) => props.onVisibilityChange(props.selectedSection, value)} />
...
function HeroEditor(props: Parameters<typeof SectionEditor>[0]) {
  return <section className={styles.card}><p className={styles.cardEyebrow}>Hero content</p><div className={styles.formGrid}><TextField label="Eyebrow" name="hero.eyebrow" onChange={props.onChange} registerField={props.registerField} value={props.content.hero.eyebrow} /><TextField label="Title" name="hero.title" onChange={props.onChange} registerField={props.registerField} value={props.content.hero.title} /><TextAreaField label="Paragraph" name="hero.paragraph" onChange={props.onChange} registerField={props.registerField} rows={5} value={props.content.hero.paragraph} /></div><div className={styles.inlineGroup}><CtaEditor heading="Primary CTA" prefix="hero.primaryCta" button={props.content.hero.primaryCta} onChange={props.onChange} registerField={props.registerField} /><CtaEditor heading="Secondary CTA" prefix="hero.secondaryCta" button={props.content.hero.secondaryCta} onChange={props.onChange} registerField={props.registerField} /></div><ImageEditor image={props.content.hero.image} onImageChange={(image) => props.onImageChange("hero.image", image)} onImageUpload={props.onImageUpload} path="hero.image" registerField={props.registerField} uploadState={props.uploadStates["hero.image"]} /></section>;
}
...
function ImageEditor(props: { path: string; image: HomePageImage; onImageChange: (image: HomePageImage) => void; onImageUpload: (path: string, file: File) => Promise<void>; registerField: SharedProps["registerField"]; uploadState?: UploadStatus; }) {
  ...
  return <div className={styles.groupPanel}><strong>Image</strong><div className={styles.homepageImageEditorGrid}><div className={styles.homepageImagePreviewFrame}>{isRenderableImageSrc(props.image.src) ? <Image alt={props.image.alt || "Homepage image preview"} fill sizes="220px" src={props.image.src} /> : <span className={styles.thumbnailFallback}>Add a valid image URL</span>}</div><div className={styles.stack}><TextField label="Image URL" name={`${props.path}.src`} onChange={(_, value) => props.onImageChange({ ...props.image, src: String(value) })} registerField={props.registerField} value={props.image.src} /><TextField label="Alt text" name={`${props.path}.alt`} onChange={(_, value) => props.onImageChange({ ...props.image, alt: String(value) })} registerField={props.registerField} value={props.image.alt} /><TextField label="Cloudinary public ID" name={`${props.path}.publicId`} onChange={(_, value) => props.onImageChange({ ...props.image, publicId: String(value) })} registerField={props.registerField} value={props.image.publicId} /><label className={styles.formField}><span>Upload / replace image</span><input accept="image/*" onChange={onFileChange} type="file" /></label>{props.uploadState?.message ? <p className={props.uploadState.status === "error" ? styles.homepageUploadError : styles.homepageUploadMessage}>{props.uploadState.message}</p> : null}</div></div></div>;
}
